...
void *sum_thread(void *vargp)
{
	...
	printf("       sum(): Thread #%ld returned with local sum of %lld\n", pthread_self(),nth_sum );
	return (void *) nth_sum;
}
void sum(long long limit, int n)
{
	...
	for (int i = 0; i < n; i++)
	{
		pthread_join(thread[i], &local_sum[i]);
		;
	}
	
	for (int i = 0; i < n; i++)
	{
		calculated_sum = calculated_sum + (long long)local_sum[i];
